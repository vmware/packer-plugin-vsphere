...
func TestDatasource_Execute(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectFailure:  false,
			expectName:     "w01-cl01-vsan01",
			expectFree:     int64(50 * units.GB),
			expectCapacity: int64(200 * units.GB),
			config: Config{
				Name: "w01-cl01-vsan01",
			},
		},
		{
			name:          "no datastore matches name",
			expectFailure: true,
			config: Config{
				Name: "does-not-exist",
			},
		...
		},
		{
			...
			expectFailure:  false,
			expectName:     "w01-cl01-nfs01",
			expectFree:     int64(900 * units.GB),
			expectCapacity: int64(1000 * units.GB),
			config: Config{
				NameRegex: "^w01-cl01-nfs[0-9]+$",
			},
		},
		{
			...
			expectFailure: true,
			config: Config{
				NameRegex: "^w01-cl01-vsan[0-9]+$",
			},
		},
		{
			name:           "most_free_space among regex matches",
			expectFailure:  false,
			expectName:     "w01-cl01-vsan02",
			expectFree:     int64(300 * units.GB),
			expectCapacity: int64(500 * units.GB),
			config: Config{
				NameRegex:     "^w01-cl01-vsan[0-9]+$",
				MostFreeSpace: true,
			},
